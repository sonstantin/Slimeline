...
class Netzplaner:
    ...
    def exit(self, event=None):
        saveornot = messagebox.askyesno("Verlassen", "Willst du vorher speichern?")
        if saveornot == True:
            self.save_plan()
            print("Beenden")
            self.master.destroy()
    ...
    def add_intermediate_stop(self, station, event=None):
        if self.build_mode:
    ...
    def lineinfo(self, line):
        ...
        info.title(f"Information zur {line[0][0]}")
        name = tk.Label(info, text=f"Name: {line[0][0]}")
        color = tk.Label(info, text=f"Farbe: {line[-1]}", bg=f"{line[-1]}")
